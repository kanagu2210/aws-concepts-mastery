...
- Decorative elements (hard shadows, dense grids) are permitted
  but treated as seasoning — used once or twice on a page,
...
- Section heading sizes must follow a stable hierarchy, not random
  variation. Vary colour and weight — not size. Unpredictable heading
  sizes force the eye to recalibrate and break reading flow.
- Within-page variation for Pause markers must be content-driven,
  not position-driven. The visual weight should match the cognitive
  weight of the pause moment in the argument.
- Pull quotes and emphasis elements must be earned — used once per
  page maximum, only for the single most important conceptual pivot.
  Using emphasis on multiple sentences devalues all of them.
...
- Paragraph spacing: minimum 1.4em between paragraphs. Paragraphs
  that run together without breathing room compress the reading pace
  and make the page feel like a wall of text.
- Container padding: minimum 2.5rem on the sides. Tight margins
  make a reading column feel like a narrow corridor.
...
Section headings within a page must follow a stable size hierarchy.
All `##` headings at the same level use the same size.
Vary colour and weight between headings — not size.
Size variation across headings at the same level is visual noise,
not design. It forces the eye to recalibrate at every heading and
disrupts reading flow across a long session.
...
**Pause markers:** If two or more Pause markers exist, render each
one differently. First might be a full-width warm tinted block with
generous padding. Second might be a left-border-only strip, quieter.
Third might be set as plain italic text with extra vertical space —
no box at all. The visual weight should match the cognitive weight
of the pause moment in the argument.
...
**`[Think: ...]` markers**
A directed reflection moment — the reader sits with a scenario
before the teaching continues.
Apply the within-page variation rule.
The full text must be visible. Do not truncate.
...
**Rotation:** Never. Do not rotate any element.
Rotated elements feel like floating objects when scrolling
and break reading flow in both browser and PDF. This is a
hard ban — not a seasoning choice.
...
A page with no shadows and no colour blocks
is a valid and often the best choice for a long reading session.

---

# LANDING LINE — FINAL MOMENT

The landing line is the last spoken sentence — the one the reader
carries out of the session. It must feel like the final word, not
like another box.

Design rules for the landing line:
- More vertical space above it than any other element on the page —
  it arrives after a breath, not immediately after the last paragraph.
- Slightly larger than body text — 110–120% of body size. Not a
  heading. Not a banner. Just slightly larger and given space.
- No competing box treatment — do not make it look like a Board
  marker or a Pause marker. It is neither. It is the conclusion.
- One accent colour touch is permitted — a single left border, or
  the text in the accent colour. Not both.
- The word "landing line" never appears on the page. It has no label.
  It is simply the last thing the reader reads, treated as the
  most important sentence it is.
...
- `transform: rotate()` — supported by WeasyPrint but BANNED by design rules (see above)

Do NOT use:
- `transform: rotate()` — banned. No element may be rotated.
...
* [Think:] marker text preserved exactly.
...
* Decorative elements (shadows) used sparingly — maximum 3 per page.
